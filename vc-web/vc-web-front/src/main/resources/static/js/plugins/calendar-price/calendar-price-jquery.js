...
(function ($) {
  ...
  fn.createCalendar = function () {
    ...
    if (setStartMonth >= thisMonthYM) {
      //showPrevBtn = false; // 注释掉 mod by jhy 仍然显示
    }
    ...
    if (setEndMonth <= thisMonthYM) {
      //showNextBtn = false; // 注释掉 mod by jhy 仍然显示
    }
  ...
  fn.initSettingWindow = function () {
    ...
    html += '               <label class="drw-enable"><input name="enableDateRange" type="checkbox"> 启用范围</label>';
    html += '           </div>';
    html += '       </div>';
    ...
    html += '       <button class="btn-confirm" id="dayDataSave">启用本设置</button>';
  ...
  fn._createDaySetupInputGroup = function () {
    ...
    for (var i = 0; i < config.length; i++) {
	  ...
	  if(val.validate){
		  html += '   <input name="'+ val.key +'" type="text" validate="'+val.validate+'">';
	  }else{
