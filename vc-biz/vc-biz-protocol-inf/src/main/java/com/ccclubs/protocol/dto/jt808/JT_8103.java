...
public class JT_8103 implements IMessageBody {
  ...
  public final byte[] WriteToBytes() {
    ...
    for (ParameterItem item : getParameters()) {
      ...
      if (fieldType.equals("BYTE") && !StringUtils.empty(strParamValue)) // 参数值类型为byte
      {
      ...
      // ORIGINAL LINE: case "WORD":
          ...
          && !StringUtils.empty(strParamValue)) // 参数值类型为16位无符号整形数值
      {
      ...
      // ORIGINAL LINE: case "DWORD":
          ...
          && StringUtils.empty(strParamValue) == false) // 参数值类型为32位无符号整形数值
      {
