...
public class JT_0200 implements IMessageBody {
  ...
  public final short getGpsCount() {
    ...
    if (addtionalItem != null) {
      PostitionAdditional_GpsCount csq = (PostitionAdditional_GpsCount) addtionalItem;
      return csq.getCount();
    } else {
      return 0;
    }
  ...
  public final short getCsq() {
    ...
    if (addtionalItem != null) {
      PostitionAdditional_Csq csq = (PostitionAdditional_Csq) addtionalItem;
      return csq.getSignal();
    } else {
      return 0;
    }
  }
  ...
  public final void setAdditionals(java.util.ArrayList<IPositionAdditionalItem> value) {
    additionals = value;
  }
  ...
  public final void ReadFromBytes(byte[] bytes) {
    ...
    setTime(
        "20" + String.format("%02X", timeBytes[0]) + "-" + String.format("%02X", timeBytes[1]) + "-"
            + String.format("%02X", timeBytes[2]) + " "
            + String.format("%02X", timeBytes[3]) + ":" + String.format("%02X", timeBytes[4]) + ":"
    ...
    while (buff.hasRemain()) {
      ...
      IPositionAdditionalItem item = PositionAdditionalFactory
          .CreatePositionalFactory(additionalId, additionalLength, additionalBytes);
      if (item != null) {
        getAdditionals().add(item);
      } else {
//				logger.info("未知的附加协议:" + additionalId + ",附加长度:" + additionalLength);
      }
  ...
  public String toString() {
    ...
    sBuilder.append(String
        .format("报警标示：%1$s,状态：%2$s,经度：%3$s,纬度：%4$s,高度：%5$s,速度：%6$s,时间：%7$s", getAlarmFlag(),
            getStatus(), getLongitude(), getLatitude(),
