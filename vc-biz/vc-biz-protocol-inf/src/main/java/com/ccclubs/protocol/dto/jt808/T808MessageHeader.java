...
public class T808MessageHeader {
  ...
  public final void ReadFromBytes(byte[] headerBytes) {
    ...
    if (BitConverter.IsLittleEndian) {
      ...
      simId = (String.format("%02X", headerBytes[4]) + String.format("%02X", headerBytes[5])
          + String.format("%02X", headerBytes[6])
          + String.format("%02X", headerBytes[7]) + String.format("%02X", headerBytes[8]) + String
          .format("%02X", headerBytes[9]));
      ...
      setSimId(String.format("%02X", simIdBytes[0]) + String.format("%02X", simIdBytes[1]) + String
          .format("%02X", simIdBytes[2])
          + String.format("%02X", simIdBytes[3]) + String.format("%02X", simIdBytes[4]) + String
          .format("%02X", simIdBytes[5]));
