...
public class JT_0802 implements IMessageBody {
	...
	public final void setDataItems(java.util.ArrayList<MuldimediaSearchDataItem> value) {
		dataItems = value;
	}
	...
	public final void ReadFromBytes(byte[] bytes) {
		...
		while (pos < bytes.length) {
			...
			if (JT808Common.isNew808Protocol()) {
				item.setMultimediaId((int) ((bytes[pos + 0] << 24) + (bytes[pos + 1] << 16) + (bytes[pos + 2] << 8) + bytes[pos + 3]));
