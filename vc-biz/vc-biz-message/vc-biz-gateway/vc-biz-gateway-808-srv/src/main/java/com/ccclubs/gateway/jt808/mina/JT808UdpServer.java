...
public class JT808UdpServer implements IJT808Server {
  ...
  private static Logger logger = LoggerFactory.getLogger(JT808UdpServer.class);
  ...
  public void stop() {
    if (null != dataAcceptor) {
      ...
      dataAcceptor
          .dispose(); // 可以另写一个类存储IoAccept，通过spring来创建，这样调用dispose后也会重新创建一个新的。或者可以在init方法内部进行创建。
      dataAcceptor = null;
    }
